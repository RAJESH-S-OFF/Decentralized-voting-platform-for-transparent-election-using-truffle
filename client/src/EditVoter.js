...
const EditVoter = ({ user,addNotification ,toPage}) => {
  const [name, setName] = useState(user.name);
  const [location, setLocation] = useState(user.location);
  const [email, setEmail] = useState(user.email);
  const [aadhar, setAadhar] = useState(user.aadhaar_no);
  const [password, setPassword] = useState(user.password); // State for password
  const [showPassword, setShowPassword] = useState(false); // State for toggling password visibility
  const [passwordValid, setPasswordValid] = useState(true); // State for password validation
  ...
  const handleSubmit =async (e) => {
    ...
    try {
      ...
      if (response.status === 201) {
        alert("Voter modification Successfully Submitted");
        toPage("/profile");
      }
    } catch (error) {
  ...
  return (
    <div className="container my-5">
      <h2 className="text-center mb-4">Edit  Voter info </h2>
